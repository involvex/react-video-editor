...
class Audio extends Trimmable {
	...
	public renderToOffscreen(force?: boolean) {
		...
		ctx.clearRect(0, 0, this.offscreenCanvas?.width ?? 0, this.height);

		// Clip with rounded corners
		ctx.beginPath();
		ctx.roundRect(0, 0, this.offscreenCanvas?.width ?? 0, this.height, this.rx);
		ctx.clip();
		...
		for (let i = startBarIndex; i < endBarIndex && i < bars.length; i++) {
			...
			if (bar) {
				...
				if (x >= 0 && x < (this.offscreenCanvas?.width ?? 0)) {
